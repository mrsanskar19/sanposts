...
export class MultiConnection {
    ...
    async insertOne<T extends Document = Document>(
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            try {
                if (this.maxDocumentsPerDB !== Infinity) {
                ...
                console.error(`[MultiConnection] insertOne failed on node:`, err.message);
                node.status = "offline";
            }
        }
    ...
    async findPaginated<T extends Document = Document>(
        collection: string,
        query: any,
        page: number,
        limit: number
        ...
        const [data, total] = await Promise.all([
            this.find<T>(collection, query, { skip, limit }),
            this.countDocuments(collection, query)
        ]);

        return { data: data as WithId<T>[], total };
    }
    ...
    async exists<T extends Document = Document>(
        collection: string,
        query: any
    ): Promise<boolean> {
    ...
    async upsertOne<T extends Document = Document>(
        collection: string,
        query: any,
        update: any
    ): Promise<UpdateResult> {
